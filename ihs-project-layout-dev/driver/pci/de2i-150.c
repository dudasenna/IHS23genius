...
static long int my_ioctl(struct file*, unsigned int cmd, unsigned long arg)
{
	switch(cmd){
	case RD_SWITCHES:
		read_pointer = bar0_mmio + 0xF0A0; //TODO: update offset
	...
	case RD_PBUTTONS:
		...
		break;
	case WR_L_DISPLAY:
		write_pointer = bar0_mmio + 0xC140; //TODO: update offset
	...
	case WR_R_DISPLAY:
		...
		break;
	case WR_RED_LEDS:
		write_pointer = bar0_mmio + 0xC0B0; //TODO: update offset
		wr_name_idx = IDX_DISPLAYR;
		break;
	case WR_GREEN_LEDS:
		write_pointer = bar0_mmio + 0xC0F0; //TODO: update offset
		wr_name_idx = IDX_DISPLAYR;
		break;
	default:
